    ...
    adLoader.prototype = {
            ...
            for(i in ads.ADTECH_MultiAd) {
                ...
                if (src.indexOf(this.config.emptySelector) === -1) {
                    for (placement in this.config.placements) {
                        if (this.config.placements[placement].id == parseInt(ad.PlacementId, 10)) {
                            placement = this.config.placements[placement];
                            container = placement.container || document.getElementById('ad-'+ placement.name);
                            break;
                        }
                    }
                    if (container) {
        ...
        init: function() {
            ...
            for (i in this.config.placements) {
                placements.push(this.config.placements[i].id);
            }
